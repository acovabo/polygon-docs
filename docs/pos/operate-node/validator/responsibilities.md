...
* Sync changes to Polygon staking contracts on Ethereum:
  ...
  * In this case, there is an `ack/no-ack` process that is followed to ensure that the next checkpoint contains a snapshot of the previous Bor blocks as well. For example, if checkpoint 1 is for Bor blocks 1-256, and it failed for some reason, the next checkpoint 2 will be for Bor blocks 1-512. See also [Heimdall architecture: Checkpoint](https://docs.polygon.technology/pos/architecture/heimdall/checkpoints/).
* State sync from the Ethereum mainnet to Bor:
  ...
  * A similar mechanism is in place for sending state from Polygon to Ethereum. See also [State Sync Mechanism](https://docs.polygon.technology/pos/architecture/bor/state-sync/).
